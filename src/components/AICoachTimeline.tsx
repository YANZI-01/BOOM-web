...
export function AICoachTimeline() {
  ...
  const timelineData = [
    ...
    },
    {
      ...
      desc: t("很多时候，影响恢复的并不是训练本身，而是训练之外持续累积的生活与工作压力。AICoach持续评估生活负荷、运动负荷与恢复状态之间的关系，并根据身体状态变化动态调整建议，帮助你避免在错误的时间进行高强度训练。", "Often, what affects recovery isn't the training itself, but the accumulated life and work stress outside of training. AICoach continuously evaluates the relationship between life load, training load, and recovery status, dynamically adjusting suggestions based on body state changes to help you avoid high-intensity training at the wrong time."),
